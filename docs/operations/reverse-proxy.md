...
| API `8080`, MCP `8081` | **Private.** Caddy reaches them over the internal `nom-indexer` docker network by service name (`api:8080`, `mcp:8081`). Bind their host publishes to loopback. |
| API metrics `9090`, MCP metrics `9091` | **Private — never proxy these.** `/metrics` is unauthenticated. They live on separate ports, so fronting `8080`/`8081` never exposes them. |
| Postgres `5432` | **Private.** Bind to loopback; never expose it. |
...
bare VPS. Two robust fixes, ideally both:

1. **Bind published ports to `127.0.0.1`** in `docker-compose.yml` so Docker
   only listens on loopback (Caddy still reaches the containers via the
   docker network). Provider-agnostic.
2. **Use a cloud security group** — it filters at the network edge, before
   the host's Docker rules, so it *does* block Docker-published ports.

Apply this to `docker-compose.yml`:
